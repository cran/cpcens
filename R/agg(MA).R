...
#'  after generating censored data from MA model. We use PELT
...
#' . And then we add this data matrix column wise and
#'   use this as a first argument in PELT.ma function.
...
PELT.ma <- function (data, pen=200*log(dim(data)[2])) {
  n = length(data)
