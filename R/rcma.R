...
#'
#'@description Randomly Generate Censored MA
...
#' ans = rcma (n=100 , ma=0.2, mu=0 , siga=1, rates=c(NA,0.7), Mrate=0 )

#' #example (left censoring)
#' ans = rcma (n=100 , ma=0.7, mu=0 , siga=1, rates=c(0.3,NA), Mrate=0 )
#' #example (interval censoring)
#' ans = rcma (n=100 , ma=0.7, mu=0 , siga=1, rates=c(0.1,0.3), Mrate=0 )
#' @importFrom stats arima.sim
#' @importFrom stats quantile

#'@export rcma
rcma<-function (n=100 , ma=0.6, mu=0 , siga=1, rates=c(NA,0.2), Mrate=0 )
{
  Rates <- rates
  if (is.vector(rates))
